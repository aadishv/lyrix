...
import { MinimalComment } from "@/hooks";
import { commentsFocus } from "@/components/comment/state";
import { useAtom } from "@xstate/store/react";
import Confirm from "../Confirm";
import { DialogClose } from "@/components/ui/dialog";
import { type Comment } from "@/components/comment/CommentHighlight";
...
export default function CommentView({
  ...
  comment: Comment;
  setComment: ({ title, content }: { title: string; content: string }) => Promise<any>;
  deleteComment: () => void;
}) {
  ...
  const handleSave = () => {
    void (async () => {
      await setComment({ title: title.trim(), content: content.trim() });
      setEditMode(false);
    })();
  };
  ...
  return (
    <div
      className="bg-white rounded-xl border-2 p-3 w-full m-0"
      ...
      {comment.linked && (
        <span className="flex text-sm">
          ...
          <span className="my-auto">
            ...
            <a className="underline" href={`/song_v2?id=${comment.linked._id}`}>
              {comment.linked.track.name}
            </a>
          </span>
        </span>
      )}
...
//   );
// }
export function CommentReadonly(args: any) {
